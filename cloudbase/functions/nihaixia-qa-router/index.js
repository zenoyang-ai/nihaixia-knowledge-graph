...
function parseAllowedOrigins(env) {
  const raw = (env.ALLOWED_ORIGINS || 'https://www.nihaixia-knowledge.xyz,https://zenoyang-ai.github.io,https://zeno-d9g0gdvw4a57635c0-1452182285.tcloudbaseapp.com,http://localhost:8765,http://127.0.0.1:8765');
  ...
  async function tryYuanqi(normalized, requestId, startTime) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const reply = normalizeOrderedListMarkers(normalizeLearningClaims((choice && choice.message && choice.message.content) || '')).trim();
...
${buildAnswerStyleInstruction(query)}
...
${context}`;
      ...
      // AI 生成常超过 SDK 默认 15s，与 MP 对齐加长超时，避免偶发 request timeout
      const app = _cloudbase.init({ env: envId, timeout: 55000 });
      const ai = app.ai();
      ...
      const reply = normalizeOrderedListMarkers(normalizeLearningClaims(result && result.text ? result.text : '')).trim();
      ...
      providerHealth.hybrid.last_success = new Date().toISOString();
      console.log(JSON.stringify({ request_id: requestId, provider: 'hybrid', status: 200, elapsed, docs_found: docs.length, reply_length: reply.length, context_chars: usedChars }));
  ...
  async function tryCloudBase(normalized, requestId, startTime) {
    ...
    try {
      ...
      const elapsed = Date.now() - startTime;
      text = normalizeOrderedListMarkers(normalizeLearningClaims(text)).trim();
...
const defaultRouter = createRouter({
  env: {
    ...
    ALLOWED_ORIGINS: process.env.ALLOWED_ORIGINS || 'https://www.nihaixia-knowledge.xyz,https://zenoyang-ai.github.io,https://zeno-d9g0gdvw4a57635c0-1452182285.tcloudbaseapp.com,http://localhost:8765,http://127.0.0.1:8765',
  },
});
